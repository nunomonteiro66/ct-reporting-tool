...
test('column-order-manual-change', async ({ page }) => {
  ...
  //unselect three attributes
  //FIRST IS TO FOCUS ON THE DROPDOWN
  await page
  ...
  const dragIds = await page
    .getByTestId('selected-columns-panel')
    .locator('[data-rfd-drag-handle-draggable-id]')
    .evaluateAll((els) =>
      els.map((el) => el.getAttribute('data-rfd-drag-handle-draggable-id'))
    );

  await expect(dragIds).toEqual(columnOrdererValues);

  //close the column orderer
  await page.getByRole('button', { name: 'Close' }).click();

  //apply column filter in column 'key'
  await page
  ...
  await expect(dragIds2).toEqual(columnOrdererValues2);
